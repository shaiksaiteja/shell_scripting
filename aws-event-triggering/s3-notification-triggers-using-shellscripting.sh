...
#################
#Author: saiteja 
#I created the shell script to enable automatic triggering of a Lambda function.
#when a user uploads a file to the bucket, sending email notifications accordingly
#################
...
# Setting variables AWS region and bucket name
aws_region="us-east-1"
bucket_name="saiteja-ultimate-bucket"
lambda_func_name="s3-lambda-function"
role_name="s3-lambda-sns-iamrole"
email_address="shaiksaiteja7095@gmail.com"

# Create IAM Role for the project #attaching role to specified services
...
aws lambda add-permission \
  --function-name "$lambda_func_name" \
  --statement-id "s3-lambda-sns-iamrole" \
...
topic_arn=$(aws sns create-topic --name s3-lambda-sns-iamrole --output json | jq -r '.TopicArn')
